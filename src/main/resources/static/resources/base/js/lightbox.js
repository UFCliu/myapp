...
(function() {
	...
	var LightboxOptions = (function() {
		...
		/*LightboxOptions.prototype.albumLabel = function(curImageNum, albumSize) {
			return "第 " + curImageNum + "张 /一共 " + albumSize + "张";
		};*/
	...
	var Lightbox = (function() {
		...
		Lightbox.prototype.enable = function() {
			...
			$('body').on(
                ...
                function(event) {
                    if($(event.currentTarget).attr('href') == "javascript:void(0);"){
                        return false;
                    }
		...
		// Show overlay and lightbox. If the image is part of a set, add
		// siblings to album array.
		Lightbox.prototype.start = function($link) {
			...
			// Support both data-lightbox attribute and rel attribute
			// implementations
		...
		Lightbox.prototype.changeImage = function(imageNumber) {
			...
			this.$lightbox
					.find(
							'.lb-image, .lb-nav,.lb-dataContainer,.lb-caption')
					.hide();
			...
			preloader.onload = function() {
				...
				if (self.options.fitImagesInViewport) {
					...
					// Take into account the border around the image and an
					// additional 10px gutter on each side.
					...
					if ((preloader.width > maxImageWidth)
						...
						if ((preloader.width / maxImageWidth) > (preloader.height / maxImageHeight)) {
							...
							imageHeight = parseInt(preloader.height
									/ (preloader.width / imageWidth), 10);
							$image.width(imageWidth);
							$image.height(imageHeight);
						} else {
							imageHeight = maxImageHeight;
							imageWidth = parseInt(preloader.width
									/ (preloader.height / imageHeight), 10);
							$image.width(imageWidth);
							$image.height(imageHeight);
						}
					}
		...
		// Display the image and it's details and begin preload neighboring
		// images.
		...
		Lightbox.prototype.updateNav = function() {
			// Check to see if the browser supports touch events. If so, we take
			// the conservative approach
			// and assume that mouse hover events are not supported and always
			...
			/*if (this.album.length > 1) {
				if (this.options.wrapAround) {
					if (alwaysShowNav) {
						this.$lightbox.find('.lb-prev, .lb-next').css(
								'opacity', '1');
					}
		...
		Lightbox.prototype.updateDetails = function() {
			...
			if (typeof this.album[this.currentImageIndex].title !== 'undefined'
				...
				this.$lightbox.find('.lb-caption').html("")
						.fadeIn('fast').find('a').on('click', function(event) {
			...
			this.$lightbox.find('.lb-dataContainer').fadeIn(
					this.options.resizeDuration, function() {
