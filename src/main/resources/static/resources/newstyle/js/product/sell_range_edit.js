...
$(document).ready(function(){
	...
	if(sellRangeSpan != null && sellRangeSpan != ''){ // 如果销售区域的字符串不为空
		...
		// 取得该供应商即存的配送范围list
		$.ajax({
			url : "/product/getTranAreaList.ajax",
			type: "post",
			async : false,
			data :{
				productId : productId,
			...
			success : function(data) {
				...
				if(data[0].provinceId == -1){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].streetId != "" && data[i].streetId != null){
							html += '<th class="thS xianCss"><input style="width:70px;display:none" type="text" id="streetId' + num + '" name ="tranAndSellRangeModelList[' + num + '].streetId" value="'+data[i].streetId+'" readonly/>'+
									'<p style="font-weight: normal;">'+ data[i].streetName +'</p>'+
									'</th>';
						}else{
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (result == 'nullError') {
				...
				if (result.length >= 25) {
					$("#sellRangeSpan").val(result.substring(0, 25) + '...');
				} else {
