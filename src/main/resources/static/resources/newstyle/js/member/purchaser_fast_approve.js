	...
	var approveRemark = $("#approveRemark").val();//审核备注
		...
		$(function(){
			...
			$(".submitBtn").click(function(){
					...
					//一级审核校验资质信息
                    var validQualInfo = "false";
					if(approveState=='OPERATION_AUDIT'){
                        $.ecpost({
                            url: "/member/fastApprove/validAlreadyQualInfo.ajax",
                            data:{"enterpriseId":$("#id").val()},
                            dataType: "json",
                            cache: false,
                            success: function(data) {
                                validQualInfo  = data.result;
                            }
                        });
						if(validQualInfo == "false"){
							openMessage("请帮助用户完善资质的有效期信息后再通过审核。");
							return ;
						}
					}
					...
					if(checkEnterpriseCode){
						...
						$.ecpost({
							...
							success: function(data) {
								if(data.msg == "success") {
									openHref(member_message_00005,"/member/fastApprove/toList.jhtml?userType="+userTypeEnum );//审核完成！
								}
