var JSWK = {
    ...
    getWinInfo: function () {
        return {
            oWidth: function () {
                return Math.max(document.documentElement.clientWidth, document.body.clientWidth, document.documentElement.offsetWidth, document.body.offsetWidth);
            },
            oHeight: function () {
                return Math.max(document.documentElement.clientHeight, document.body.clientHeight, document.documentElement.offsetHeight, document.body.offsetHeight);
            },
            cWidth: function () {
            ...
            zWidth: function () {
                return Math.max(document.documentElement.clientWidth, document.body.clientWidth, document.documentElement.offsetWidth, document.body.offsetWidth, document.documentElement.scrollWidth, document.body.scrollWidth);
            },
            zHeight: function () {
                return Math.max(document.documentElement.clientHeight, document.body.clientHeight, document.documentElement.offsetHeight, document.body.offsetHeight, document.documentElement.scrollHeight, document.body.scrollHeight);
            }
        };
    },
    ...
    lockScreen: function (f) {
        ...
        if (objsbgDiv) {
            ...
            if (navigator.userAgent.indexOf("MSIE 6.0") > 0) {
                ...
                b.style.cssText = "position:absolute;width:" + d + "px;height:" + c + "px;left:0px;top:0px; z-index:9;filter:alpha(opacity=0);opacity:0;";
                document.body.appendChild(b)
            }
            var a = document.createElement("div");
            a.setAttribute("id", "JSWK_bgDiv");
            a.style.cssText = "position:absolute;top:0px;left:0px;background:#333333;width:" + d + "px;height:" + c + "px;z-index:10;";
            JSWK.opacity(a, f);
    ...
    createDivIframe: function (g, k, e, f, c) {
        ...
        if (Jid("JSWK_CreateDiv")) {
            ...
            m.innerHTML = "";
            if (e == "true") {
                m.innerHTML += "<div id='theMoveArea'  moveArea='true' style='cursor: move;' class='" + c + "'>" + f + "</div>";
            } else {
                m.innerHTML += "<div id='theMoveArea'  moveArea='false' style='cursor: move; display:none;'></div>";
            }
            m.innerHTML += "<iframe id='JSWK_CreateIframe' application='yes' frameborder='0' scrolling='no' marginheight='0' marginwidth='0' name='JSWK_CreateIframe' width='" + l + "' height='" + b + "' src='' onload='JSWK.initframeHeight(this);JSWK.setNewH(this);' style='margin:0px;padding:0px;'></iframe>";
            m.style.zIndex = "15";
    ...
    autoToSize: function (a, j, k) {
        ...
        var d = g.getChilds();
        if (d.length < 1) {
            g.appChild("<p style='width:" + g.clientWidth + "px; height:" + g.clientHeight + "px; visibility:hidden;'>&nbsp;</p>");
        }
        d = g.getFirst();
        var b = sH = 0;
        b = (j - g.clientWidth) / f;
    ...
    fadeframeIn: function (b, c, a) {
        JSWK.scrollEffect.animate(b, "in:alpha:" + c, a);
        JSWK.fadeframeOut = "show";
    },
    fadeframeOut: function (b, c, a) {
        JSWK.scrollEffect.animate(b, "out:alpha:" + c, a);
        JSWK.fadeframeOut = "hide";
    },
    fadeframeState: "hide",
    switchFade: function (b, c, a) {
        if (JSWK.curBox != null && JSWK.curBox != Jid(b)) {
            JSWK.fadeframeOut(JSWK.curBox.id, c, a);
        }
        if (JSWK.fadeframeOut == "hide") {
            JSWK.fadeframeIn(b, c, a);
        } else {
            if (JSWK.fadeframeState == "show") {
                JSWK.fadeframeOut(b, c, a);
            }
        }
    },
    slidebaseDown: function (b, c, a) {
        JSWK.scrollEffect.animate(b, "down:" + c, a);
        JSWK.slidebaseState = "show";
    },
    slidebaseUp: function (b, c, a) {
        JSWK.scrollEffect.animate(b, "up:" + c, a);
        JSWK.slidebaseState = "hide";
    },
    slidebaseState: "hide",
    switchSlidebase: function (b, c, a) {
        if (JSWK.curBox != null && JSWK.curBox != Jid(b)) {
            JSWK.slidebaseUp(JSWK.curBox.id, c, a);
        }
        if (JSWK.slidebaseState == "hide") {
            JSWK.slidebaseDown(b, c, a);
        } else {
            if (JSWK.slidebaseState == "show") {
                JSWK.slidebaseUp(b, c, a);
            }
        }
    },
    ...
    initframeHeight: function (e) {
        if (e.contentDocument) {
            var a = Math.max(e.contentDocument.body.offsetWidth, e.contentDocument.documentElement.offsetWidth, e.contentDocument.body.scrollWidth, e.contentDocument.documentElement.scrollWidth);
            e.width = a + "px";
            e.parentNode.style.width = a + "px";
            var d = Math.max(e.contentDocument.body.offsetHeight, e.contentDocument.documentElement.offsetHeight, e.contentDocument.body.scrollHeight, e.contentDocument.documentElement.scrollHeight);
            e.height = d + "px";
            e.parentNode.style.height = d + "px"
        } else {
            if (e.Document) {
                var c = Math.max(e.Document.body.offsetWidth, e.Document.documentElement.offsetWidth, e.Document.body.scrollWidth, e.Document.documentElement.scrollWidth);
                e.width = c + "px";
                e.parentNode.style.width = c + "px";
                var b = Math.max(e.Document.body.offsetHeight, e.Document.documentElement.offsetHeight, e.Document.body.scrollHeight, e.Document.documentElement.scrollHeight);
                e.height = b + "px";
